-- =====================================================================
-- Migration 42: filter by plan — broadcast tool + customer directory
-- =====================================================================
-- Adds an optional p_plan_id to broadcast_candidates, staff_list_
-- customers, and staff_search_customers. A plan match means either:
--   - a recurring plan: child_subscriptions.plan_id = p_plan_id, or
--   - a special plan:   a child_special_passes row for that plan
-- Passing null keeps existing behavior (no filtering) for every caller
-- that doesn't know about this yet.
-- =====================================================================
...
as $$
  with base as (
    select
      ...
      cs.plan_id as recurring_plan_id,
      ...
      exists (
        ...
        where csp.child_id = ch.id and csp.plan_id = p_plan_id
      ) as has_special_pass
    from customers cu
  ...
  from (
    ...
    where
      ...
      and (p_plan_id is null or b.recurring_plan_id = p_plan_id or b.has_special_pass)
  ) results;
$$;

grant execute on function broadcast_candidates(
  text, int, text, int, int, int, int, int, int, uuid
) to authenticated;

-- ---------------------------------------------------------------------
-- Customer directory — same plan-match rule, but gates which CUSTOMERS
-- appear rather than filtering their children list (a parent with one
-- child on the filtered plan still sees their other kids too).
-- ---------------------------------------------------------------------
